...
def generate_dummy_data(chunk_size=5, anomaly_probability=0.05):
    ...
    for city_id in unique_city_ids:
        ...
        if generate_anomaly:
            ...
            anomaly_value_watt = random.choice([random.randint(0, 250), random.randint(300, 400)])
...
def send_data_over_socket(host='localhost', port=9999, interval_seconds=5):
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.bind((host, port))
    s.listen(1)
    print(f"Listening for connections on {host}:{port}")

    accumulated_data = []  # To store accumulated data across batches
    ...
    while True:
        ...
        try:
            while True:
                ...
                accumulated_data.extend(chunk.to_dict(orient='records'))
